...
import {
  useApproveRecruiterReview,
  useRecruiterPendingReviews,
  useRejectRecruiterReview,
} from '@/modules/companies/hooks';

export function EmployerReviewsPage() {
  const { data, isLoading } = useRecruiterPendingReviews(1, 20);
  const approveMutation = useApproveRecruiterReview();
  const rejectMutation = useRejectRecruiterReview();
